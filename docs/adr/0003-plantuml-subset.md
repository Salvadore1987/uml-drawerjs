...
| `Rel(from, to, "label", "tech"?)`, plus `Rel_U/D/L/R`                     | `EdgeKind: "uses"`. `label` carries the action name; the 4th argument maps to the dedicated `DiagramEdge.technology` field (rendered as a second `[tech]` line). The legacy `[tech]`-suffix encoding is still decoded by the generator for back-compat. |
