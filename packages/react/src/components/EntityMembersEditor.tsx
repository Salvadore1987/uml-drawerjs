...
/**
 * Built-in column types offered in the type-select dropdown. Names cover the
 * common UML / ER spellings plus database-shaped types (`Text`, `JSON`,
 * `Binary`) so the dropdown reads naturally for ER modelling without
 * picking a single SQL dialect.
 */
...
/**
 * Live list of types eligible for entity columns: built-in primitives plus
 * every entity label currently in the diagram (so a column can reference
 * another entity by name when modelling foreign keys with embedded types).
 */
              ...
              <li key={attr.id} className="uml-class-members__row">
                ...
                >
                  <input
                    ...
                    onChange={(e): void =>
                      replace(index, (prev) =>
                        e.target.checked ? { ...prev, foreignKey: true } : omit(prev, "foreignKey"),
                      )
                    }
                  />
                  <span>FK</span>
